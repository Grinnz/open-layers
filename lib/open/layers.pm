...
implementation has several quirks that may be useful to know:
...
Perl 5.14, pushing a C<:crlf> layer on top of other translation layers on
...
The C<:utf8> layer sets a flag that indicates the preceding set of layers will
translate the input to Perl's internal upgraded string format, which may
resemble UTF-8 or UTF-EBCDIC, or will translate the output from that format. It
is B<not> an encoding translation layer; for this, use C<:encoding(UTF-8)> or
L<PerlIO::utf8_strict>. Any encoding translation layer will generally set the
C<:utf8> flag, even when the desired encoding is not UTF-8, as they translate
between the desired encoding and Perl's internal format. (The C<:bytes> layer
unsets this flag, thus indicating the stream is to be interpreted as bytes.)
