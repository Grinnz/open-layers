...
open::layers - Set default PerlIO layers, or set PerlIO layers on standard
filehandles
...
redesigned to be more explicit and intuitive.
...
Unlike C<open.pm>, C<open::layers> requires that the target of the operation is
always specified so as to not confuse global and lexical operations.

=item *

Unlike C<open.pm>, C<open::layers> can set layers on the standard handles
without affecting handles opened in the lexical scope.

=item *

Unlike C<open.pm>, multiple layers are not required to be space separated.

=item *

Unlike C<open.pm>, the C<:locale> pseudo-layer is not (yet) implemented.
